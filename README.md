...
4. Before editing a FHIR Core resource, the agent asks the user to confirm the
   exact release-note heading or label unless the request already supplies it.
...
- For FHIR Core, the user-confirmed release-note section contains the ticket
  exactly once and historical ballot-cycle notes remain unchanged.
...
that ticket commit also includes its entry under the user-confirmed
resource-page release-note label.
